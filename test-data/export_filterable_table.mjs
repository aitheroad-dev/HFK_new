// Export all test people to a filterable HTML table
// Usage: cd /Users/yaronkra/Jarvis/projects/hkf-crm && node test-data/export_filterable_table.mjs
...
try {
  ...
  const outputPath = join(__dirname, 'test_people_filterable_table.html');
  writeFileSync(outputPath, html);
  console.log(`\n✅ Filterable table saved to: ${outputPath}`);
...
function generateHTML(people) {
  ...
  const jsonData = JSON.stringify(people.map((p, idx) => ({
    ...
    appliedAt: p.enrollment?.appliedAt ? new Date(p.enrollment.appliedAt).toLocaleDateString('en-GB') : '',
    enrolledAt: p.enrollment?.enrolledAt ? new Date(p.enrollment.enrolledAt).toLocaleDateString('en-GB') : '',
    ...
    interviewScheduledAt: p.interview?.scheduledAt ? new Date(p.interview.scheduledAt).toLocaleDateString('en-GB') : '',
    ...
    paymentAmount: p.payment?.amount ? `${p.payment.currency} ${(p.payment.amount / 100).toFixed(2)}` : '',
    paymentMethod: p.payment?.method || '',
    paidAt: p.payment?.paidAt ? new Date(p.payment.paidAt).toLocaleDateString('en-GB') : ''
  })));
...
<head>
  ...
  <title>HKF CRM - Test People Filterable Table</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-interviewing { background: #ede9fe; color: #5b21b6; }
    .badge-accepted { background: #d1fae5; color: #065f46; }
    .badge-rejected { background: #fee2e2; color: #991b1b; }
    .badge-enrolled { background: #a7f3d0; color: #047857; }
    .badge-completed { background: #6ee7b7; color: #047857; }
    .badge-dropped { background: #e5e7eb; color: #374151; }
    .badge-scheduled { background: #dbeafe; color: #1e40af; }
    .badge-no_show { background: #fee2e2; color: #991b1b; }
    .badge-passed { background: #d1fae5; color: #065f46; }
    .badge-failed { background: #fee2e2; color: #991b1b; }
    .badge-pending_decision { background: #fef3c7; color: #92400e; }
...
<body>
  <header>
    <h1>HKF CRM - Test People Filterable Table</h1>
    <p>Generated: ${new Date().toLocaleString('en-GB')} | Filter by any column</p>
  </header>
  ...
  <div class="controls">
    ...
    <div class="filter-group">
      ...
      <select id="filterEnrollmentStatus">
        ...
        <option value="_none">No Enrollment</option>
      </select>
    </div>
    ...
    <div class="filter-group">
      ...
      <select id="filterInterviewStatus">
        ...
        <option value="_none">No Interview</option>
      </select>
    </div>
    ...
    <div class="filter-group">
      ...
      <select id="filterPaymentStatus">
        ...
        <option value="_none">No Payment</option>
      </select>
    </div>
  ...
  <div class="stats-bar">
    <span>Showing: <strong id="visibleCount">${people.length}</strong> of <strong>${people.length}</strong> people</span>
    <span>With Enrollment: <strong>${people.filter(p => p.enrollment).length}</strong></span>
    <span>With Interview: <strong>${people.filter(p => p.interview).length}</strong></span>
    <span>With Payment: <strong>${people.filter(p => p.payment).length}</strong></span>
  </div>
  ...
  <div class="table-container">
    <table id="peopleTable">
      <thead>
        <tr>
          <th data-col="num">#</th>
          <th data-col="firstName">First Name<input type="text" data-filter="firstName" placeholder="Filter..."></th>
          <th data-col="lastName">Last Name<input type="text" data-filter="lastName" placeholder="Filter..."></th>
          <th data-col="email">Email<input type="text" data-filter="email" placeholder="Filter..."></th>
          <th data-col="phone">Phone<input type="text" data-filter="phone" placeholder="Filter..."></th>
          <th data-col="personStatus">Status</th>
          <th data-col="city">City<input type="text" data-filter="city" placeholder="Filter..."></th>
          <th data-col="source">Source<input type="text" data-filter="source" placeholder="Filter..."></th>
          <th data-col="occupation">Occupation<input type="text" data-filter="occupation" placeholder="Filter..."></th>
          <th data-col="age">Age</th>
          <th data-col="enrollmentStatus">Enrollment</th>
          <th data-col="appliedAt">Applied</th>
          <th data-col="motivation">Motivation<input type="text" data-filter="motivation" placeholder="Filter..."></th>
          <th data-col="interviewStatus">Interview</th>
          <th data-col="interviewOutcome">Outcome</th>
          <th data-col="interviewScore">Score</th>
          <th data-col="interviewRecommendation">Recommendation<input type="text" data-filter="interviewRecommendation" placeholder="Filter..."></th>
          <th data-col="interviewLocation">Location<input type="text" data-filter="interviewLocation" placeholder="Filter..."></th>
          <th data-col="paymentStatus">Payment</th>
          <th data-col="paymentAmount">Amount</th>
          <th data-col="paymentMethod">Method</th>
        </tr>
      </thead>
      <tbody id="tableBody">
      </tbody>
  ...
  <script>
    ...
    function renderTable(filteredData) {
      ...
      tbody.innerHTML = filteredData.map(p => \`
        <tr>
          ...
          <td>\${p.phone || '<span class="no-data">-</span>'}</td>
          <td><span class="\${getBadgeClass(p.personStatus)}">\${p.personStatus || '-'}</span></td>
          ...
          <td>\${p.enrollmentStatus ? \`<span class="\${getBadgeClass(p.enrollmentStatus)}">\${p.enrollmentStatus}</span>\` : '<span class="no-data">-</span>'}</td>
          <td>\${p.appliedAt || '<span class="no-data">-</span>'}</td>
          <td>\${p.motivation || '<span class="no-data">-</span>'}</td>
          <td>\${p.interviewStatus ? \`<span class="\${getBadgeClass(p.interviewStatus)}">\${p.interviewStatus}</span>\` : '<span class="no-data">-</span>'}</td>
          <td>\${p.interviewOutcome ? \`<span class="\${getBadgeClass(p.interviewOutcome)}">\${p.interviewOutcome}</span>\` : '<span class="no-data">-</span>'}</td>
          ...
          <td>\${p.paymentStatus ? \`<span class="\${getBadgeClass(p.paymentStatus)}">\${p.paymentStatus}</span>\` : '<span class="no-data">-</span>'}</td>
    ...
    function exportCSV() {
      ...
      const headers = ['#', 'First Name', 'Last Name', 'Email', 'Phone', 'Status', 'City', 'Source', 'Occupation', 'Age', 'Enrollment', 'Applied', 'Motivation', 'Interview', 'Outcome', 'Score', 'Recommendation', 'Location', 'Payment', 'Amount', 'Method'];
      const rows = filtered.map(p => [
        p.num, p.firstName, p.lastName, p.email, p.phone, p.personStatus, p.city, p.source, p.occupation, p.age,
        p.enrollmentStatus, p.appliedAt, p.motivation, p.interviewStatus, p.interviewOutcome, p.interviewScore,
        p.interviewRecommendation, p.interviewLocation, p.paymentStatus, p.paymentAmount, p.paymentMethod
      ]);
      ...
      a.href = url;
      a.download = 'test_people_filtered.csv';
      a.click();
    }
